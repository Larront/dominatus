	...
	const groups = $derived(
		SCORING_GROUPS.map((g) => ({
			...
			rows: g.categories
				...
				.map((c) => ({ value: `+${profile[c.key]}`, label: c.label, note: c.hint, sub: threshold(c) }))
...
<main class="mx-auto w-full max-w-2xl px-6 py-8 max-[720px]:px-4">
	<header class="mb-7">
		...
		<h1 class="mt-1.5 font-display text-2xl font-bold tracking-[0.02em] text-accent">Campaign rules</h1>
		<p class="mt-2.5 max-w-prose font-prose text-[15px] leading-[1.6] text-ink-dim">
			...
			standings are the points — two separate tallies, both derived from the reports you log. This is
			the Malvernis Sector charter.
		</p>
	</header>

	<!-- ── World control ─────────────────────────────────────────────────────── -->
	<section class="mb-9">
		<h2 class="flex items-baseline gap-2 border-b border-border pb-2 font-display text-[11px] font-semibold tracking-[0.14em] text-ink-dim uppercase">
			<span class="text-accent">▸</span> World control
		</h2>

		<p class="mt-3.5 font-prose text-[15px] leading-[1.6] text-ink">
			Every world is a fixed <b class="text-accent">100% pool</b>. Each warband holds a share in steps
			of 10%; whatever no one holds is the world's <b class="text-ink">uncontested</b> remainder. Worlds
			start fully uncontested. A decisive battle moves control by <b class="text-accent">10 points</b>:
		</p>
		...
		<ul class="mt-3 flex flex-col gap-2 font-body text-[13px] leading-[1.55] text-ink-dim">
			<li class="flex gap-2.5">
				...
				<span>The <b class="text-ink">loser</b> drops 10%, floored at zero — a warband never goes negative.</span>
			</li>
			<li class="flex gap-2.5">
				<span class="mt-[7px] size-1 shrink-0 bg-accent" aria-hidden="true"></span>
				<span>The <b class="text-ink">winner</b> rises 10%, taken from the loser first, then from the uncontested pool — that is how a world is first claimed. The gain is capped so the world never exceeds 100%.</span>
			</li>
			<li class="flex gap-2.5">
				<span class="mt-[7px] size-1 shrink-0 bg-state-defender" aria-hidden="true"></span>
				<span>In a <b class="text-ink">2v2</b> the rule applies per warband: each winner +10%, each loser −10%, so a winning side pulls up to 20% off the world.</span>
			</li>
			<li class="flex gap-2.5">
		...
		<div class="mt-5 grid grid-cols-3 gap-2.5 max-[560px]:grid-cols-1">
			<div class="border border-state-defender-line bg-state-defender-soft px-3.5 py-3">
				<p class="font-display text-[11px] font-semibold tracking-[0.08em] text-state-defender uppercase">Owned</p>
				<p class="mt-1.5 font-body text-[12px] leading-[1.5] text-ink-dim">One warband holds a majority — over 50%.</p>
			</div>
			<div class="border border-state-contested-line bg-state-contested-soft px-3.5 py-3">
				<p class="font-display text-[11px] font-semibold tracking-[0.08em] text-state-contested uppercase">Contested</p>
				<p class="mt-1.5 font-body text-[12px] leading-[1.5] text-ink-dim">Shares exist, but no warband holds a majority.</p>
			</div>
			<div class="border border-border bg-panel-2/60 px-3.5 py-3">
				<p class="font-display text-[11px] font-semibold tracking-[0.08em] text-ink-faint uppercase">Unclaimed</p>
				<p class="mt-1.5 font-body text-[12px] leading-[1.5] text-ink-dim">No warband holds any share of the world.</p>
			</div>
		</div>
		...
		<p class="mt-5 font-prose text-[15px] leading-[1.6] text-ink">
			How big an army you may bring is set by control of the world you fight over. The cap follows the
		...
		<div class="mt-3 border border-border bg-[color-mix(in_srgb,var(--color-panel)_70%,transparent)]">
			<p class="border-b border-border px-3.5 py-2.5 font-display text-[9.5px] font-medium tracking-[0.12em] text-ink-faint uppercase">
				...
				<tbody>
					{#each ladder as row (row.band)}
						<tr class="border-t border-border first:border-t-0">
							...
							<td class="px-3.5 py-2.5 text-right font-semibold tabular-nums text-accent">{row.size}</td>
						</tr>
					{/each}
				</tbody>
			</table>
		...
		<p class="mt-2.5 font-body text-[12px] leading-[1.5] text-ink-faint">
			A table-talk rule, agreed before the game — the cogitator does not check it, and reports submit
			at any size.
		</p>
	</section>
	...
	<section class="mb-9">
		...
		<p class="mt-3.5 font-prose text-[15px] leading-[1.6] text-ink">
			The leaderboard is a points table, <b class="text-accent">separate from control</b>. Most points
			fall straight out of the reports you log; painting is granted by the arbiter. Points accrue per
			warband — a commander running two warbands scores them apart. The arbiter sets each category's
			value; anything switched off doesn't appear here.
		</p>
		...
		{#each groups as group (group.title)}
			<div class="mt-5">
				<p class="font-display text-[10px] font-semibold tracking-[0.12em] text-accent uppercase">
					{group.title}
					<span class="ml-1 font-body text-[11px] tracking-normal text-ink-faint normal-case">— {group.blurb}</span>
				</p>
				<dl class="mt-1.5 flex flex-col">
					{#each group.rows as row (row.label)}
						<div class="grid grid-cols-[64px_1fr] items-baseline gap-3 border-t border-border py-3 first:border-t-0 max-[480px]:grid-cols-1 max-[480px]:gap-1">
							<dt class="font-body text-[15px] font-semibold tabular-nums text-accent">{row.value}</dt>
							<dd>
								<span class="font-display text-[12px] font-semibold tracking-[0.06em] text-ink uppercase">{row.label}</span>
								{#if row.sub}<span class="ml-2 font-body text-[11px] text-ink-faint">{row.sub}</span>{/if}
	...
	<section>
		...
		<p class="mt-3.5 font-prose text-[15px] leading-[1.6] text-ink-dim">
			The campaign runs in numbered cycles, and every report is stamped with the cycle it was fought in.
			This campaign is on <b class="text-accent">cycle {data.campaign.currentCycle}</b>. How long a cycle
			lasts, and what happens when one closes, are the arbiter's call.
		</p>
	</section>
</main>
