...
<div class="relative flex min-h-[100dvh] flex-col overflow-hidden bg-void">
	...
	<div aria-hidden="true" class="pointer-events-none absolute inset-0">
		...
		></div>
		<div
			class="absolute inset-0 bg-[linear-gradient(var(--color-grid-line)_1px,transparent_1px),linear-gradient(90deg,var(--color-grid-line)_1px,transparent_1px)] [mask-image:radial-gradient(ellipse_at_center,#000_30%,transparent_78%)] bg-[size:46px_46px]"
		></div>
	...
	<header class="relative z-10 flex items-center gap-4 px-[26px] py-4 max-[680px]:px-4">
		<span class="flex items-center gap-[11px]">
			<BrandMark
				class="size-8 shrink-0 text-accent drop-shadow-[0_0_6px_var(--color-accent-glow)]"
			/>
	...
	<!-- Hero -->
	<main
		class="relative z-10 mx-auto grid w-full max-w-[1140px] flex-1 grid-cols-1 items-center gap-x-12 gap-y-10 px-[26px] py-10 max-[680px]:px-4 lg:grid-cols-[1.05fr_0.95fr]"
	>
		...
		<div class="planet-in order-1 flex justify-center lg:order-2">
			<div class="relative grid aspect-square w-[min(80vw,420px)] place-items-center">
				...
				<div
					class="pointer-events-none absolute inset-0 [animation:spin_24s_linear_infinite] motion-reduce:hidden"
				>
					<span
				...
				<Planet
					...
					size="min(58vw,300px)"
					resolution={220}
					name="A world of the contested system"
				/>
			</div>
		</div>
		...
		<div class="order-2 flex flex-col items-start lg:order-1">
			<p
				class="rise font-display text-[11px] font-semibold tracking-[0.34em] text-accent uppercase"
			>
				// Campaign Cogitator
			</p>
			<h1
				class="rise mt-3 font-display text-[clamp(2.9rem,9vw,5.25rem)] leading-[0.92] font-bold tracking-[-0.01em] text-balance text-ink"
				style="--d: 60ms"
			>
