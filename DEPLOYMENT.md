...
- [x] `docker-compose.yml` with host bind mount `/data/dominatus/data` mounted at `/data` (and `/data/dominatus/backups` at `/backups`); host dirs must be owned by the `bun` UID 1000
...
- [x] **Uploaded scoresheets persist on the data volume** — battle-report images are written to `<DATABASE_URL dir>/images` (i.e. `/data/images`), so the existing `/data` bind mount (`/data/dominatus/data`) covers them; no new volume needed. Stored on submit as evidence for the confirmed report (ADR 0001), served via the campaign-gated `/campaigns/[slug]/report/image/[file]`. Cleaned up on report delete/replace. `BODY_SIZE_LIMIT` (below) must stay above the 12 MB image cap.
