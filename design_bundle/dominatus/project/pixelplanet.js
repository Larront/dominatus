...
(function () {
	...
	function tick() {
		...
		instances.forEach((inst) => inst._draw(t));
		rafId = instances.size ? requestAnimationFrame(tick) : null;
	}

	class PixelPlanet {
		constructor(canvas, type, opts) {
			opts = opts || {};
			this.canvas = canvas;
			...
			// programs (cache per-kind on the context)
			this._progs = {};
			this.layers = recipe(type).map((L) => {
				if (!this._progs[L.kind]) this._progs[L.kind] = makeProgram(gl, FRAGS[L.kind]);
				return { prog: this._progs[L.kind], scale: L.scale, u: L.u };
			});
	...
	window.PixelPlanet = PixelPlanet;
	window.PixelPlanet.FRAGS = FRAGS;
	window.PixelPlanet.VERT = VERT;
	window.PixelPlanet.HEAD = HEAD;
})();
